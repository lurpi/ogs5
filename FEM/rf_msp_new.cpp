    ...
    while (!new_keyword)
    {
        ...
        if (line_string.find("$SWELLING_PRESSURE_TYPE") != string::npos)
        {
            ...
            else if (SwellingPressureType == 3 || SwellingPressureType == 4)
            {
                ...
                {
                    std::cout << "No multi-phase flow coupled. The thermal "
                                 "elatic model can only be used in H2 coupled "
                                 "proccess."
                              << "\n";
        ...
        if (line_string.find("CAPACITY") != string::npos)
        {
            ...
            switch (Capacity_mode)
            {
                ...
                case 5:  // Capacity depending on solid conversion
                    // 0. Capacity at lower_density_limit (reactive system
                    // property)
                    // 1. Capacity at upper_density_limit (reactive system
                    // property)
                ...
                case 6:  // Capacity depending on loading with adsorbate
                    ...
                    in_sd.clear();
                    break;
                case 7:  // Capacity depending on solid conversion
        ...
        if (line_string.compare("CONDUCTIVITY") == 0)
        {
            ...
            switch (Conductivity_mode)
            {
                ...
                case 4:  //  = f(S) //21.12.2009 WW
                    ...
                    break;
                case 5:  // DECOVALEX2015, Task B2, Buffer: f(S,T) by matrix
                         // function
                    in_sd >> T_0;
        ...
        {  // subkeyword found
            ...
            switch (type)  // 15.03.2008 WW
            {
                ...
                case 1000:  // case 10-13: transverse isotropic linear
                    ...
                    //  direction) 2: nu_{ia} (Poisson's ratio w.r.t. the
                    //  anisotropy direction) 3: G_a     (shear modulus w.r.t.
                    //  the anisotropy direction) 4: n_x     (x-coefficient of
                    //  the local axis of anisotropy (2D case: -\sin\phi)) 5:
                    //  n_y     (y-coefficient of the local axis of anisotropy
                    //  (2D case: \cos\phi)) 6: n_z     (z-coefficient of the
                    //  local axis of anisotropy (2D case: 0))
        ...
        if (line_string.find("$TIME_DEPENDENT_YOUNGS_POISSON") != string::npos)
        {
            ...
            {
                case 1:  // isotropic
                    in_sd >> Time_Dependent_E_nv_value[0] >>
                        Time_Dependent_E_nv_value[1];  //[0] for E, [1] for nv
                    break;
                case 2:  // transversely isotropic
                    in_sd >> Time_Dependent_E_nv_value[0] >>
                        Time_Dependent_E_nv_value[1] >>
                        Time_Dependent_E_nv_value[2] >>
                        Time_Dependent_E_nv_value[3] >>
                        Time_Dependent_E_nv_value[4];
                    //[0]: Ei, [1]: Ea, [2]: ni, [3]: Eia, [4]: Ga
                    break;
                default:
                    cout << "!ERROR in msp file, no valid "
                            "TIME_DEPENDENT_YOUNG_POISSON mode!"
                         << endl;
                    break;
            }
            in_sd.clear();
        ...
        if (line_string.find("BISHOP_COEFFICIENT") != string::npos)  // WX
        {
            ...
            switch (bishop_model)
            {
                ...
                case 2:  // pow(Se, parameter)
                    in_sd >> bishop_model_value;
                    break;
                case 3:  // JM model 3:    if p<bishop_model_value ->
        ...
        if (line_string.find("$REACTIVE_SYSTEM") != string::npos)
        {
            ...
            this->setSolidReactiveSystem(
                FiniteElement::convertSolidReactiveSystem(reaction_system));
            in_sd.clear();
            if (reaction_system.compare("SINUSOIDAL") == 0)
            {  // For Benchmarks
                in_sd.str(GetLineFromFile1(msp_file));
                in_sd >> reaction_enthalpy;  // in J/kg, negative for exothermic
        ...
        {
            in_sd.str(GetLineFromFile1(msp_file));
            in_sd >> non_reactive_solid_volume_fraction >>
                non_reactive_solid_density;
            in_sd.clear();
        }
        ...
        if (line_string.find("$ENTHALPY_CORRECTION_REFERENCE_TEMPERATURE") !=
            string::npos)
        {
            in_sd.str(GetLineFromFile1(msp_file));
                                        ...
                                        // conversion_rate::conversion_rate
{
    if (reaction_system.compare("CaOH2") == 0)
    {
        ...
        reaction_enthalpy =
            -1.083e+05 /
            PhysicalConstant::MolarMass::Water;  // in J/kg (Molar heat of
                                                 // reaction divided by molar
                                                 // mass of water)
        // negative for exothermic composition reaction
        reaction_entropy =
            -143.5 / PhysicalConstant::MolarMass::Water;  // in J/kgK
        std::cout
            << "Set enthalpy correction reference temperature with keyword "
               "$ENTHALPY_CORRECTION_REFERENCE_TEMPERATURE; usually to initial "
               "solid temperature."
            << std::endl;
    }
    else if (reaction_system.compare("Mn3O4") == 0)
    {
        ...
        upper_solid_density_limit =
            4860.0;  // density Mn3O4 (Mangan(II,III)-oxid)
        reaction_enthalpy =
            -1.376e+05 / 0.032;  // in J/kg (Molar heat of reaction divided by
                                 // molar mass of oxygen)
        // negative for exothermic composition reaction
        reaction_entropy = -114.1 / 0.032;  // in J/kgK
        std::cout
            << "Set enthalpy correction reference temperature with keyword "
               "$ENTHALPY_CORRECTION_REFERENCE_TEMPERATURE; usuall to initial "
               "solid temperature."
            << std::endl;
    }
    else if (reaction_system.compare("Z13XBF") == 0)
    {
        ...
        upper_solid_density_limit =
            1.e6;                 // state dependent; not needed for calculation
...
/**************************************************************************
   ...
   const Task: Get heat capacity Programing: 08/2004 WW Implementation
   **************************************************************************/
double CSolidProperties::Heat_Capacity(double refence)
{
    ...
    switch (Capacity_mode)
    {
        ...
        case 1:
            ...
            break;
        case 3:
            val = 1.38 * (refence - PhysicalConstant::CelsiusZeroInKelvin) +
                  732.5;
            break;
        case 4:  // solid capacity depending on solid density (for
                 // thermochemical heat storage) - TN
            // refence contains value of solid density (current)
            val = (*data_Capacity)(0) +
                  ((*data_Capacity)(1) - (*data_Capacity)(0)) /
                      ((*data_Capacity)(3) - (*data_Capacity)(2)) *
        ...
        case 6:  // in a sorption system
        {
            ...
            val = lower_solid_density_limit / refence *
                  ((*data_Capacity)(0) + C * (*data_Capacity)(1));
        }
        break;
        case 7:
            // linear density average (compare model 5)
            val = lower_solid_density_limit * (*data_Capacity)(0);
            val += (upper_solid_density_limit * (*data_Capacity)(1) -
                    lower_solid_density_limit * (*data_Capacity)(0)) /
                   (upper_solid_density_limit - lower_solid_density_limit) *
                   (refence - lower_solid_density_limit);
            val /= refence;
            break;
        default:
        {
            cout << "***Error in CSolidProperties::Heat_Capacity(double "
                    "refence):"
                    "heat capacity mode "
                 << Capacity_mode << " is not supported";
        }
    }
    return val;
}
...
/**************************************************************************
   FEMLib-Method: CSolidProperties::Heat_Capacity(const double reference = 0.0)
    ...
    switch (Conductivity_mode)
    {
        ...
        case 2:
        {
            ...
            if (reference < k_T[2])  // Wet
                ...
                val =
                    k_T[0] + (k_T[0] - k_T[1]) * (reference - k_T[2]) / k_T[3];
            else
                val = k_T[1];
        }
        break;
        ...
        case 5:  // DECOVALEX2015, TaskB2 JM
            ...
            val = GetMatrixValue(primary_variable[0] + T_0, primary_variable[1],
                                 name, &gueltig);
            break;
        default:
        {
            cout << "***Error in CSolidProperties::Heat_Conductivity(double "
                    "reference):"
                    "heat conductivity mode "
                 << Conductivity_mode << " is not supported";
            abort();
        }
    }
    return val;
                                              ...
                                              int group)
{
    ...
    switch (Conductivity_mode)
    {
        ...
        case 3:  // DECOVALEX THM1
            ...
            break;
        case 5:
            base_thermal_conductivity = GetMatrixValue(
                primary_variable[1], primary_variable[0] + T_0, name, &gueltig);
            break;
        default:  // Normal case
        {
            cout << "***Error in CSolidProperties::HeatConductivityTensor(): "
                    "conductivity mode is not supported "
                 << "\n";
            abort();
        }
    }
    ...
    if (thermal_conductivity_tensor_type > 0 &&
        ...
        cout << "***Error in CSolidProperties::HeatConductivityTensor(): "
                "problem dimension and the given tensor "
...
/**************************************************************************
   ...
   Task: general routine to get 6x6 consistent tangent from local Newton
iteration of material functionals Programing: 06/2014 TN Implementation
**************************************************************************/
void CSolidProperties::ExtractConsistentTangent(
    const Eigen::MatrixXd& Jac, const Eigen::MatrixXd& dGdE,
    const bool pivoting, Eigen::Matrix<double, 6, 6>& dsigdE)
{
    ...
    // solve linear system
    if (pivoting)
        dzdE = Jac.fullPivHouseholderQr().solve(
            -1.0 * dGdE);  // Could consider moving to different Eigen solver.
    // dzdE = Jac.fullPivLu().solve(-1.0 * dGdE); // Could consider moving to
    // different Eigen solver.
    else
        dzdE = Jac.householderQr().solve(
            -1.0 * dGdE);  // Could consider moving to different Eigen solver.
...
/**************************************************************************
   ...
   03/2015 NB Modified
**************************************************************************/
void CSolidProperties::LocalNewtonBurgers(
    const double dt, const std::vector<double>& strain_curr,
    ...
    double Temperature, double& local_res)
{
    ...
    sigd_j = 2.0 * (epsd_i - eps_M_t -
                    eps_K_t);  // initial guess as elastic predictor
    ...
    material_burgers->CalResidualBurgers(dt, epsd_i, sigd_j, eps_K_j, eps_K_t,
                                         eps_M_j, eps_M_t, res_loc);
    // initial evaluation of Jacobian
    material_burgers->CalJacobianBurgers(
        dt, K_loc, sig_eff, sigd_j,
        eps_K_j);  // Note - With constant properties a single evaluation would
                   // be sufficient.
    ...
    //    for (int counter(0); counter<counter_max && res_loc.norm() >
    //    local_tolerance; ++counter)
    while (res_loc.norm() > Tolerance_Local_Newton && counter < counter_max)
    {
        counter++;
        // Solve linear system
        // inc_loc = K_loc.fullPivHouseholderQr().solve(res_loc); //other linear
        // solvers (faster but less accurate) can be considered. inc_loc =
        // K_loc.fullPivLu().solve(res_loc); //other linear solvers (faster but
        // less accurate) can be considered inc_loc =
        ...
        material_burgers->CalResidualBurgers(
            dt, epsd_i, sigd_j, eps_K_j, eps_K_t, eps_M_j, eps_M_t, res_loc);
        // Get Jacobian
        material_burgers->CalJacobianBurgers(
            dt, K_loc, sig_eff, sigd_j,
            eps_K_j);  // for solution dependent Jacobians
    }
    ...
    sig_j = material_burgers->GM * sigd_j +
            material_burgers->KM * e_i * SolidMath::ivec;
    dsigdE = material_burgers->GM * dsigdE * SolidMath::P_dev +
             3. * material_burgers->KM * SolidMath::P_sph;

    // Sort into Consistent Tangent matrix for global Newton iteration and into
    // standard OGS arrays
    ...
    for (size_t i = 0; i < 3; i++)
    {
        for (size_t j = 0; j < 3; j++)
        {
            ...
            Consistent_Tangent(i, j + 3) =
                dsigdE(i, j + 3) / sqrt(2.);  // from local to global shear
                                              // components (Kelvin to Voigt)
            Consistent_Tangent(i + 3, j) =
                dsigdE(i + 3, j) / sqrt(2.);  // from local to global shear
                                              // components (Kelvin to Voigt)
...
**************************************************************************/
void CSolidProperties::LocalNewtonMinkley(
    const double dt, const std::vector<double>& strain_curr,
    ...
    double& local_res)
{
    ...
    // dimensionless stresses
    sigd_j = 2.0 * (epsd_i - eps_M_j - eps_K_j -
                    eps_pl_j);  // initial guess as elastic predictor
    // Calculate effective stress and update material properties
    sig_eff = SolidMath::CalEffectiveStress(sigd_j);
    ...
    // initial evaluation of residual and Jacobian
    material_minkley->CalViscoelasticResidual(dt, epsd_i, e_i, e_pl_v, sig_j,
                                              eps_K_j, eps_K_t, eps_M_j,
                                              eps_M_t, eps_pl_j, res_loc);
    // initial evaluation of Jacobian
    ...
    {
        counter++;
        // Solve linear system; Choice of solver can be influenced by material
        // properties/property ratios
        ...
        material_minkley->UpdateMinkleyProperties(sig_eff, e_pl_eff,
                                                  Temperature);
        // evaluation of new residual
        material_minkley->CalViscoelasticResidual(
            dt, epsd_i, e_i, e_pl_v, sig_j, eps_K_j, eps_K_t, eps_M_j, eps_M_t,
            eps_pl_j, res_loc);
        // Get Jacobian
        material_minkley->CalViscoelasticJacobian(dt, sig_j, sig_eff, K_loc);
    }
    if (!(material_minkley->YieldMohrCoulomb(sig_j * material_minkley->GM) <
          Tolerance_Local_Newton))
    {
        Eigen::Matrix<double, 27, 1> res_loc_p, inc_loc_p;
        Eigen::Matrix<double, 27, 27> K_loc_p;
        material_minkley->CalViscoplasticResidual(
            dt, epsd_i, e_i, sig_j, eps_K_j, eps_K_t, eps_M_j, eps_M_t,
            eps_pl_j, eps_pl_t, e_pl_v, e_pl_v_t, e_pl_eff, e_pl_eff_t, lam,
            res_loc_p);
        material_minkley->CalViscoplasticJacobian(dt, sig_j, sig_eff, lam,
                                                  e_pl_eff, K_loc_p);
        while (res_loc_p.norm() > Tolerance_Local_Newton &&
               counter < 2 * counter_max)
        {
            counter++;
            // Solve linear system; Choice of solver can be influenced by
            // material properties/property ratios
    ...
    else
    {
        ...
        local_res = -res_loc.norm();  // negative to imply that viscoelastic
                                      // routine was used.
    }
    // add hydrostatic part to stress and tangent
...
void CSolidProperties::Calculate_Lame_Constant()
{
    ...
    if (Time_Dependent_E_nv_mode > 0)
    {
        ...
        {
            case 1:
                nv *= GetCurveValue(Time_Dependent_E_nv_value[1], 0,
                                    aktuelle_zeit, &valid);
                E *= GetCurveValue(Time_Dependent_E_nv_value[0], 0,
                                   aktuelle_zeit, &valid);
                break;
            case 2:
                cout << "WARNING: for isotropic elasticity, please use "
                        "TIME_DEPENDENT_YOUNG_POISSON mode=1"
                     << endl;
                break;
            default:
                cout << "ERROR: not valid TIME_DEPENDENT_YOUNG_POISSON mode."
                     << endl;
                break;
        }
    }
    ...
    const int Dimension)
{
    ...
    if (Time_Dependent_E_nv_mode > 0)
    {
        ...
        {
            case 1:
                cout << "WARNING: for transversely isotropic elasticity, "
                        "please use TIME_DEPENDENT_YOUNG_POISSON mode=2"
                     << endl;
                break;
            case 2:
                Ei *= GetCurveValue(Time_Dependent_E_nv_value[0], 0,
                                    aktuelle_zeit, &valid);
                Ea *= GetCurveValue(Time_Dependent_E_nv_value[1], 0,
                                    aktuelle_zeit, &valid);
                ni *= GetCurveValue(Time_Dependent_E_nv_value[2], 0,
                                    aktuelle_zeit, &valid);
                nia *= GetCurveValue(Time_Dependent_E_nv_value[3], 0,
                                     aktuelle_zeit, &valid);
                Ga *= GetCurveValue(Time_Dependent_E_nv_value[4], 0,
                                    aktuelle_zeit, &valid);
                break;
            default:
                cout << "ERROR: not valid TIME_DEPENDENT_YOUNG_POISSON mode."
...
void CSolidProperties::CalculateCoefficent_MOHR(
...
// 09.2011
{
    ...
    if (Plasticity_Bedding &&
        ((scalar_comp_tmp != 0) || (scalar_tens_tmp != 0)))
    {
        // csn=GetCurveValue(bedding_uc_curve,0,scalar_comp_tmp,&valid);
        // theta =
        // GetCurveValue(bedding_fric_curve,0,scalar_comp_tmp,&valid)*hPI/180;
...
int CSolidProperties::DirectStressIntegrationDPwithTension(
    const int GPiGPj, Matrix* De, const ElementValue_DM* ele_val,
    double* TryStress, const int Update, double& mm)
{
    ...
    // //WX:16.09.2010.correct H. WW sy0 = (*ele_val->y_surface)(GPiGPj);
            ...
            else
            {
                ...
                if (Update < 1)  // calculate mm for Dep
                {
                    ...
                    if (tmpI1 > 3 * tension)
                        n1 = (tmpI1 - 3 * tension) /
                             (dI2 -
                              3 * tension);  // n1 =  (I1(sigtmp)-I1(tension))/
                                             // (I1(SigB)-I1(tension))
                    else
                        n1 = 0;
                    if (dI1 != 0.0)
...
/*******************************************************
LU: Mohr Coulomb and Bedding plane = prescribed shear on one plane (anisotropy of parameters mantained for legacy)
routine is based on DirectStressIntegrationMOHR, first it checks and compute for rupture of the matrix
(DirectStressIntegrationMOHR with plasticity_beddings check removed) and then it checks for rupture on the
weakness plane (stress rotated and projected parallel and perpendicular to the weakplane normal vector)
*******************************************************/
int CSolidProperties::StressIntegrationMOHR_Joint(const int GPiGPj, const ElementValue_DM* ele_val, double* TryStress,
    const int Update, Matrix* Dep)
{
    ...
    *TmpDe = (0.);
    ...
    } // end check of Mohr-Copulomb rupture in the matrix
    else
    { // calculate shear or tension in the joint
        double normdstr = 0.;
        for (i = 0; i < Size; i++)
        {
            //dstrs[i] = TryStress[i]; // d_stress
        ...
        {
            //std::cout << "normdstr smaller than MKleinsteZahl" <<endl;
            //return 0;
        }*/
        ...
        *TmpMatrix2 = (0.);
        ...
        if (tensionsurf > 1e-5)
            ...
            shearsurf = -1; //sqrt((TryStr_buff[5] * TryStr_buff[5]) + (TryStr_buff[4] * TryStr_buff[4]));
            //std::cout << "New shearsurf on joint" << shearsurf << endl;
            //id_fs_n[2]= 0.0;
            //id_fs_s[5] =-1 ;
            //id_fs_s[4] =-1 ;
            }
        ...
        if (tensionsurf < 1e-5 && shearsurf > 1e-5 ) // || tensionsurf > 1e-5) // start check of Mohr-Copulomb rupture in the joint
        {
            ...
            double lamda_pos = 0., lamda_neg = 0., shearsurf_pos = 0, shearsurf_neg = 0;

            if (shearsurf > 1e-5)      // LU check if it works or should be larger ...
            {
                ...
                while (1)
                {
                    ...
                    if (first_step)
                    {
                        for (i = 0; i < Size; i++)
                        {
                            dfs_dsig[i] = id_fs_n[i] * tan(thetaj) + shear_cos_direction[i];
                        ...
                        if (shearsurf_k1 > 0)
                        {
                            ...
                            if (first_step)
                            {
                                ...
                                if (fabs(shearsurf_k1) < 1e-3)
                                    {
                                    std::cout << " At first step, ssurf < 1e-3 stop " << endl;
                                    break;
                                    }                                            
                                // first_step = false;
                            }
                            ...
                            {
                                dlamda = 0.;
                                //std::cout << " dlamda is 0 " <<endl;
                            }
                            else
                            {
                                dlamda = shearsurf / TmpValue1;
                                //std::cout << " dlamda is NOT 0, but " << dlamda <<endl;
                            }
                            ...
                            if (first_step)
                            {
                                ...
                                first_step = false;
                                //std::cout << " First step is now false, dlamda = " << dlamda <<endl;
                            }
                            else
                            {
                    ...
                    else
                    {
                        ...
                        else if (shearsurf_k1 < 0)
                        {
                            for(i=0;i<Size;i++)
                                dsig_dlamda[i]=0.;
                            Dep->multi(dgs_dsig,dsig_dlamda,-1);//update De dG/dsig, reactivated 13.09.2019
                            shearsurf_neg = shearsurf_k1;
                            lamda_neg = dlamda;
                            dlamda = lamda_pos - shearsurf_pos * (lamda_neg - lamda_pos) / (shearsurf_neg - shearsurf_pos);
                            //std::cout << "No, shearsurf_k1 negative, dlamda lamda_neg-lamda_pos"	<< (lamda_neg - lamda_pos) <<endl;
                        ...
                        else
                        {
                            ...
                            lamda_pos = dlamda;
                            shearsurf_pos = shearsurf_k1;
                            // LU: test to avoid mohr failure crashes 09.2019
                            
                            if (shearsurf_neg > -1.e-13)
                                        dlamda = lamda_pos + shearsurf_pos * (lamda_neg - lamda_pos) / (shearsurf_pos);
                            else
                                        dlamda = lamda_pos - shearsurf_pos * (lamda_neg - lamda_pos) / (shearsurf_neg - shearsurf_pos);
                            // LU: test to avoid mohr failure crashes 09.2019
                            //dlamda = lamda_pos - shearsurf_pos * (lamda_neg - lamda_pos) / (shearsurf_neg - shearsurf_pos);
                            //std::cout << "No, shearsurf_k1 positive, dlamda lamda_neg-lamda_pos"	<< (lamda_neg - lamda_pos) <<endl;
                            if (fabs(lamda_neg - lamda_pos) < 1e-12)
                                break;
                            // dlamda = (lamda_pos+lamda_neg)/2.;
                        }
                    ...
                    for (i = 0; i < Size; i++) // local_damp for Newton downhill
                    {
                        TmpStress[i] = TryStr_buff[i] + dlamda * dsig_dlamda[i]; // sig(n+1)=sig(try)-dlambda*De*dG/dsig
                                                                                 //std::cout << i	<<"-component TryStr_buff "	<< TryStr_buff[i] << ",new stress " << TmpStress[i] <<" stress correction " << i	<<"-component"	<< dlamda * dsig_dlamda[i] <<endl;
                    ...
                    */
                    ...
                    if (tensionsurf > 1e-5)
                    {
                            ...
                            std::cout << "But tensile surf is positive, New shearsurf_k1 on joint" << shearsurf_k1 << endl;
...
*******************************************************/
int CSolidProperties::StressIntegrationMOHR_Aniso(
    ...
    const int Update, Matrix* Dep)
{
    ...
    double dstrs[6] = {0.}, TryStress_0[6] = {0.}, TryStr_buff[6] = {0.},
           TmpStress[6] = {0.};
    double tmp_prin_str[6] = {0.}, tmp_prin_dir[9] = {0.},
           TmpPrinStrComp[6] = {0.}, TmpPrinStrTens[6] = {0.};
    double TmpStrComp[6] = {0.}, TmpStrTens[6] = {0.}, devStr[6] = {0.},
           dstrainP[6] = {0.}, dstressP[6] = {0.};
    double sqrt3 = sqrt(3.0);
    bool lode_out_range = false;
    ...
    *ConstitutiveMatrix = (0.);
    ...
    shearsurf =
        Ntheta * (I1 / 3. + 2 / sqrt3 * sqrtJ2 * sin(LodeAngle + 2 / 3. * PI)) -
        (I1 / 3. + 2 / sqrt3 * sqrtJ2 * sin(LodeAngle - 2 / 3. * PI)) - csn;
    tensionsurf =
        (I1 / 3. + 2 / sqrt3 * sqrtJ2 * sin(LodeAngle + 2 / 3. * PI)) - tension;
    ...
    if (shearsurf > 1e-5 || tensionsurf > 1e-5)
    {
        ...
        double dsig_dlamda[6] = {0.},
               /*dsig_dlamda_k1[6]={0.},*/ dfs_dsig[6] = {0.},
               dft_dsig[6] = {0.}, dgs_dsig[6] = {0.}, dgt_dsig[6] = {0.};
        bool first_step;
        double shearsurf_k1,
            tensionsurf_k1 /*, local_damp, tmp_pos = 1., tmp_neg=1.*/
            ;              // Newton downhill
        double lamda_pos = 0., lamda_neg = 0., shearsurf_pos, shearsurf_neg,
               tensionsurf_pos, tensionsurf_neg;
        if (tensionsurf > MKleinsteZahl)  // if tension
        {
            ...
            while (1)
            {
                ...
                if (Plasticity_Bedding)
                {
                    double a_pk_sig_pq_sig_kq = 0., sig_pq_sig_pq = 0.,
                           dTens_daPara = 0.;
                    ...
                    *TmpMatrix2 = (0.);
                    ...
                    for (p = 0; p < 3; p++)  // aki_sigkj, sigpq_sigpa =
                                             // sigmn_sigmn, apk_sigpq_sigkq
                    {
                        a_ki_sig_kj[0] +=
                            (*TmpMatrix)(p, 0) * (*TmpMatrix2)(p, 0);
                        a_ki_sig_kj[1] +=
                            (*TmpMatrix)(p, 1) * (*TmpMatrix2)(p, 1);
                        a_ki_sig_kj[2] +=
                            (*TmpMatrix)(p, 2) * (*TmpMatrix2)(p, 2);
                        a_ki_sig_kj[3] +=
                            (*TmpMatrix)(p, 0) * (*TmpMatrix2)(p, 1);
                        a_ki_sig_kj[4] +=
                            (*TmpMatrix)(p, 0) * (*TmpMatrix2)(p, 2);
                        a_ki_sig_kj[5] +=
                            (*TmpMatrix)(p, 1) * (*TmpMatrix2)(p, 2);
                        for (q = 0; q < 3; q++)
                        {
                            sig_pq_sig_pq +=
                                (*TmpMatrix2)(p, q) * (*TmpMatrix2)(p, q);
                            for (k = 0; k < 3; k++)
                                a_pk_sig_pq_sig_kq += (*TmpMatrix)(p, k) *
                                                      (*TmpMatrix2)(p, q) *
                                                      (*TmpMatrix2)(k, q);
                        }
                    }
                    ...
                    for (i = 1; i < bedding_tens_curve_order + 1; i++)
                        dTens_daPara += i * tens_para[i] *
                                        MathLib::fastpow(AnisoParaTens,
                                                         (i - 1));  // dcsn/deta
                    for (i = 0; i < Size; i++)
                        dtens_dsig[i] =
                            dTens_daPara *
                ...
                if (first_step)
                {
                    for (i = 0; i < Size; i++)
                    {
                        dft_dsig[i] = 1 / 3. * Kronecker[i] +
                                      2 / sqrt3 * sin(LodeAngle + 2 / 3. * PI) *
                                          dsqrtJ2_dsig[i] +
                                      2 / sqrt3 * sqrtJ2 * dlode_dsig[i] *
                                          cos(LodeAngle + 2 / 3. * PI) -
                                      dtens_dsig[i];
                        dgt_dsig[i] = 1 / 3. * Kronecker[i] +
                                      2 / sqrt3 * sin(LodeAngle + 2 / 3. * PI) *
                                          dsqrtJ2_dsig[i] +
                                      2 / sqrt3 * sqrtJ2 * dlode_dsig[i] *
                                          cos(LodeAngle + 2 / 3. * PI);
                ...
                else
                {
                    ...
                    else if (tensionsurf_k1 < 0)
                    {
                        ...
                        lamda_neg = dlamda;
                        dlamda = lamda_pos -
                                 tensionsurf_pos * (lamda_neg - lamda_pos) /
                                     (tensionsurf_neg - tensionsurf_pos);
                ...
                for (i = 0; i < Size; i++)
                    TmpStress[i] =
                        ...
                        dlamda *
                            dsig_dlamda
                                [i];  // sig(n+1)=sig(try)-dlambda*De*dG/dsig
                ...
                else
                {
                    ...
                    else if ((-3 * sqrt3 / 2. * J3 / pow(sqrtJ2, 3)) <
                             -1 + 1e-6)
                    {
                        LodeAngle = -PI / 6.;
                        lode_out_range = true;
                    ...
                    {
                        LodeAngle =
                            asin(-3 * sqrt3 / 2. * J3 / pow(sqrtJ2, 3)) / 3.;
                        lode_out_range = false;
                    }
                }
                shearsurf =
            ...
            *TmpMatrix2 = (0.);
            ...
            {
                for (j = 0; j < Size; j++)
                    (*TmpDe)(i, j) -= (*TmpMatrix2)(i, j) /
                                      TmpValue1;  // Dep = De - De dG/ds dF/ds
                                                  // De / dF/ds de dG/ds
            }
            // TEST
            // TmpDe->Write();
            ...
            while (1)
            {
                ...
                if (Plasticity_Bedding)
                {
                    ...
                    *TmpMatrix2 = (0.);
                    ...
                    for (p = 0; p < 3; p++)  // aki_sigkj, sigpq_sigpq =
                                             // sigmn_sigmn, apk_sigpq_sigkq
                    {
                        a_ki_sig_kj[0] +=
                            (*TmpMatrix)(p, 0) * (*TmpMatrix2)(p, 0);
                ...
                if (first_step)
                {
                    for (i = 0; i < Size; i++)
                    {
                        ...
                        dgs_dsig[i] = (Nphi - 1) / 3. * Kronecker[i] +
                                      2 / sqrt3 *
                                          (Nphi * sin(LodeAngle + 2 / 3. * PI) -
                                           sin(LodeAngle - 2 / 3. * PI)) *
                                          dsqrtJ2_dsig[i] +
                                      2 / sqrt3 * sqrtJ2 * dlode_dsig[i] *
                                          (Nphi * cos(LodeAngle + 2 / 3. * PI) -
                                           cos(LodeAngle - 2 / 3. * PI));
                        //-dcsn_dsig[i];
                    }
                }
                ...
                else
                {
                    ...
                    if (fabs(shearsurf_k1) < 1e-3)
                    {
                        // for(i=0;i<Size;i++)
                        //	dsig_dlamda[i]=0.;
                        // Dep->multi(dgs_dsig,dsig_dlamda,-1);//update De
                        // dG/dsig
                        break;
                    }
                    else if (shearsurf_k1 < 0)
...
double CSolidProperties::CalAnisoPara(double* Stress, double* MicroStruTensor)
{
    ...
    double l[3] = {0.};
    L1 = sqrt(Stress[0] * Stress[0] + Stress[3] * Stress[3] +
              Stress[4] * Stress[4]);
    L2 = sqrt(Stress[3] * Stress[3] + Stress[1] * Stress[1] +
              Stress[5] * Stress[5]);
    L3 = sqrt(Stress[4] * Stress[4] + Stress[5] * Stress[5] +
              Stress[2] * Stress[2]);
...
*******************************************************/
int CSolidProperties::DirectStressIntegrationMOHR(const int GPiGPj,
                                                  ...
                                                  int itesteps)
{
    ...
    *ConstitutiveMatrix = (0.);  // in head already defined, and is used for
                                 // later as global variable
    ...
    *TmpDe = (0.);
    ...
    if (Plasticity_Bedding)  // WX:09.2011
    {
        if (itesteps == 2 || Update > 0)
        {
            ...
            double Stress_Bedding_comp[6] = {0.},
                   Stress_Bedding_tens[6] = {0.};  // Stress_Bedding[6]={0.}
            ...
            double tr_sig_comp = 0., tr_sig_tens = 0., tr_a_sig_comp = 0.,
                   tr_a_sig_tens = 0.;
            ...
            //
            // tr_a_sig_comp +=
            // MicroStruTensor[i]*(Stress_Bedding_comp[i]*Stress_Bedding_comp[i]
            //+);
            // tr_a_sig_tens +=
            // MicroStruTensor[i]*Stress_Bedding_tens[i]*Stress_Bedding_tens[i];
            if (tr_sig_comp == 0)
                scalar_comp = 0;
            else
            ...
            //{
            //	cout<<"ERROR: Please check input value for BEDDING_NORM in
            //.msp"<<endl; 	exit(1);
            //}
            ...
            // tmp_load_dir_major = acos(fabs(tmp_load_dir_major))*180/PI;
            // //because ||bedding_norm|| and ||prin_dir|| is 1
        ...
        CalculateCoefficent_MOHR(ep, (*ele_val->scalar_aniso_comp)(GPiGPj),
                                 (*ele_val->scalar_aniso_tens)(GPiGPj));
    }
    else
        CalculateCoefficent_MOHR(ep, 0, 0);
...
double CSolidProperties::CalVar_t(double* vecl, double* veclg, Matrix* D,
                                  double* sigma_B, double* sigma_l, int Size)
{
    double TmpVec[6] = {0.};
    double deltaStr[6] = {0.};
    double TmpVal = 0.;
...
/**************************************************************************
 ...
 potential with respect to stresses const long EleIndex        : Element index
   const double *DevS         : The deviatoric stresses
   const double *S_Invariants : Stress invariants
   const double *MatN1        : Material parameters
...
/**************************************************************************
 ...
 potential with respect to stresses and material parameters
...
*************************************************************************/
void CSolidProperties::CalTransMatrixMicroStru(Matrix* Trans,
                                               double* bedding_norm)
{
    double tmp_dir[9] = {0.};
    ...
    for (int ii = 0; ii < 9; ii++)
    {
        if (fabs(tmp_dir[ii]) > 1)
            cout << "WARNING: Please check the input value for $BEDDING_NORM !"
                 << endl;
        break;
    }
...
double CSolidProperties::E_Function(int dim, const ElementValue_DM* ele_val,
                                    int ngp)
{
    ...
    switch (E_Function_Model)
    {
        case 1:
            ...
            return_value =
                GetCurveValue((int)E_Function_Model_Value[0], 0, I1, &valid);
            break;
        case 2:
        {
            ...
            return_value = GetCurveValue((int)E_Function_Model_Value[0], 0,
                                         prin_str[0], &valid);
            break;
        }
        case 3:
            return GetCurveValue((int)E_Function_Model_Value[0], 0,
                                 aktuelle_zeit, &valid);
        default:
            return_value = 1.;
            break;
...
void CSolidProperties::setSolidReactiveSystem(
    FiniteElement::SolidReactiveSystem reactive_system)
{
    _reactive_system = reactive_system;
}

FiniteElement::SolidReactiveSystem CSolidProperties::getSolidReactiveSystem()
    const
{
    return _reactive_system;
}

double CSolidProperties::getBishopCoefficient(const double effectiveS,
