...
std::ios::pos_type CMediumProperties::Read(std::ifstream* mmp_file)
{
	...
	while (!new_keyword)
	{
		    ...
		    && (!(line_string.find("_DISTRIBUTION") != std::string::npos)))
		{
			...
			switch (porosity_model)
			{
				...
				//case 13: // mineral precipitation / dissolution by ABM
					//in >> porosity_model_values[0]; // Initial porosity
					//break;
				case 13: // FM1 //LU:10.2016 with output br residuale opening --- bshear and  bstress directly from .rfd curve
					in >> porosity_model_values[0];  //LU : b_initial 08.2018
					in >> porosity_model_values[1];  //LU : curve number pls_e - tan(dilation angle) 11.2017
					in >> porosity_model_values[2];  //LU : curve number bstress (pressure dependent aperture)  10.2017;
					in >> porosity_model_values[3];  //LU : x-pos of inj point 12.2017;
					in >> porosity_model_values[4];  //LU : y-pos of inj point 12.2017;
					in >> porosity_model_values[5];  //LU : z-pos of inj point 12.2017;
					in >> porosity_model_values[6];  //LU : range of increased perm 12.2017;
					in >> porosity_model_values[7];  //LU : intial perm increased around the well 12.2017;
					break;
				case 14: //FM2 //LU:10.2016 FM2 --- bshear and  bstress directly from .rfd curve
					in >> porosity_model_values[0];  //LU : b_initial 08.2018
					in >> porosity_model_values[1];  //LU : curve number pls_e - tan(dilation angle) 11.2016
					in >> porosity_model_values[2];  //LU : curve number bstress (eff stress aperture) 10.2016;
					break;
				case 15: //FM1 //LU:10.2016 FM1 --- bshear and  b_pressure directly from .rfd curve
					in >> porosity_model_values[0];  //LU : b_initial 08.2018
					in >> porosity_model_values[1];  //LU : curve number pls_e - tan(dilation angle) 11.2017
					in >> porosity_model_values[2];  //LU : curve number bstress (pressure dependent aperture)  10.2017;
					in >> porosity_model_values[3];  //LU : x-pos of inj point 12.2017;
					...
					in >> porosity_model_values[7];  //LU : intial perm increased around the well 12.2017;
					break;
				case 16: //FM2 //LU:10.2016 FM2 --- bshear and  b_pressure directly from .rfd curve
					in >> porosity_model_values[0];  //LU : b_initial 08.2018
					in >> porosity_model_values[1];  //LU : curve number pls_e - tan(dilation angle) 11.2017
					in >> porosity_model_values[2];  //LU : curve number bstress (pressure dependent aperture) 10.2017;
					break;
		...
		if (line_string.find("$STORAGE") != std::string::npos)
		{
			...
			switch (storage_model)
			{
				...
				case 7: // RW/WW
					...
					val_l = storage_model_values[0] * (1. - storage_model_values[0] * storage_model_values[1])
		...
		if (line_string.find("$PERMEABILITY_FUNCTION_EFFSTRESS") != std::string::npos)
		{
			...
			switch (permeability_effstress_model)
			{
				case 1:
					...
					break;
				case 13: // FM1 //LU:10.2016 with output br residuale opening --- bshear and  bstress directly from .rfd curve
					in >> permeability_effstress_model_value[0];  //LU : b_initial 08.2018
					in >> permeability_effstress_model_value[1];  //LU : curve number pls_e - tan(dilation angle) 11.2017
					in >> permeability_effstress_model_value[2];  //LU : curve number bstress (pressure dependent aperture)  10.2017;
					in >> permeability_effstress_model_value[3];  //LU : x-pos of inj point 12.2017;
					in >> permeability_effstress_model_value[4];  //LU : y-pos of inj point 12.2017;
					in >> permeability_effstress_model_value[5];  //LU : z-pos of inj point 12.2017;
					in >> permeability_effstress_model_value[6];  //LU : range of increased perm 12.2017;
					in >> permeability_effstress_model_value[7];  //LU : intial perm increased around the well 12.2017;
					in >> permeability_effstress_model_value[8];  //LU : thickness of faut elements (needed for perm->b^3/12 transmissivity);
					break;
				case 14: //FM2 //LU:10.2016 FM2 --- bshear and  bstress directly from .rfd curve, intrinsic perm = b^3/(12*fault_thickness)
					in >> permeability_effstress_model_value[0];  //LU : b_initial 08.2018
					in >> permeability_effstress_model_value[1];  //LU : curve number pls_e - tan(dilation angle) 11.2016
					in >> permeability_effstress_model_value[2];  //LU : curve number bstress (eff stress aperture) 10.2016;
					in >> permeability_effstress_model_value[3];  //LU : thickness of faut elements (needed for perm->b^3/12 transmissivity);
					break;
				case 15: //FM1 //LU:10.2016 FM1 --- bshear and  b_pressure directly from .rfd curve
					in >> permeability_effstress_model_value[0];  //LU : b_initial 08.2018
					in >> permeability_effstress_model_value[1];  //LU : curve number pls_e - tan(dilation angle) 11.2017
					...
					in >> permeability_effstress_model_value[7];  //LU : intial perm increased around the well 12.2017;
					in >> permeability_effstress_model_value[8];  //LU : thickness of faut elements (needed for perm->b^3/12 transmissivity);
					std::cout << "mmp Model 15 FM1, open fracture of "<< permeability_effstress_model_value[6] <<" m radius centered in x " <<permeability_effstress_model_value[3] <<" y "<< permeability_effstress_model_value[4] <<" z "<< permeability_effstress_model_value[5] << '\n';
					break;
				case 16: //FM2 //LU:10.2016 FM2 --- bshear and  b_pressure directly from .rfd curve, intrinsic perm = b^3/(12*fault_thickness)
					in >> permeability_effstress_model_value[0];  //LU : b_initial 08.2018
					in >> permeability_effstress_model_value[1];  //LU : curve number pls_e - tan(dilation angle) 11.2017
					in >> permeability_effstress_model_value[2];  //LU : curve number bstress (pressure dependent aperture) 10.2017;
					in >> permeability_effstress_model_value[3];  //LU : thickness of fault elements (needed for perm->b^3/12 transmissivity);
					std::cout << "mmp Model 16 FM2" << '\n';
					break;
				case 18: //FM2 //LU:10.2016 FM2 --- bshear and  b_pressure directly from .rfd curve, intrinsic perm = b^3/(12*fault_thickness)
					in >> permeability_effstress_model_value[0];  //LU : b_initial 08.2018
					in >> permeability_effstress_model_value[1];  //LU : curve number pls_e - tan(dilation angle) 11.2017
					in >> permeability_effstress_model_value[2];  //LU : curve number bstress (pressure dependent aperture) 10.2017;
					in >> permeability_effstress_model_value[3];  //LU : thickness of fault elements (needed for perm->b^3/12 transmissivity);
					std::cout << "mmp Model 18 FM2 with rigid fracture until plastic strain activated" << '\n';
					break;
				case 22: //FM2 //LU:10.2016 FM2 --- bshear and  b_pressure directly from .rfd curve, intrinsic perm = b^2/(12) preserve permeability
					in >> permeability_effstress_model_value[0];  //LU : b_initial 08.2018
					in >> permeability_effstress_model_value[1];  //LU : curve number pls_e - tan(dilation angle) 11.2017
					in >> permeability_effstress_model_value[2];  //LU : curve number bstress (pressure dependent aperture) 10.2017;
					in >> permeability_effstress_model_value[3];  //LU : thickness of fault elements (needed for perm->b^3/12 transmissivity);
					break;
				case 21: //FM1 //LU:10.2016 FM2 --- bshear and  b_pressure directly from .rfd curve, intrinsic perm = b^3/(12) preserve permeability
					in >> permeability_effstress_model_value[0];  //LU : b_initial 08.2018
					in >> permeability_effstress_model_value[1];  //LU : curve number pls_e - strain_opening 11.2017
					in >> permeability_effstress_model_value[2];  //LU : curve number bstress (pressure dependent aperture) 10.2017;
					in >> permeability_effstress_model_value[3];  //LU : thickness of fault elements (needed for perm->b^3/12 transmissivity);
					break;
				case 32: //FM2 //LU:10.2016 FM2 --- bshear and  b_pressure directly from .rfd curve, intrinsic perm = b^2/(12) preserve permeability
					in >> permeability_effstress_model_value[0];  //LU : b_initial 08.2018
					in >> permeability_effstress_model_value[1];  //LU : curve number pls_e - tan(dilation angle) 11.2017
					in >> permeability_effstress_model_value[2];  //LU : curve number bstress (pressure dependent aperture) 10.2017;
					in >> permeability_effstress_model_value[3];  //LU : thickness of fault elements (needed for perm->b^3/12 transmissivity);
					break;
				case 31: //FM1 //LU:10.2016 FM2 --- bshear and  b_pressure directly from .rfd curve, intrinsic perm = b^3/(12) preserve permeability
					in >> permeability_effstress_model_value[0];  //LU : b_initial 08.2018
					in >> permeability_effstress_model_value[1];  //LU : curve number pls_e - strain_opening 11.2017
					in >> permeability_effstress_model_value[2];  //LU : curve number bstress (pressure dependent aperture) 10.2017;
					in >> permeability_effstress_model_value[3];  //LU : thickness of fault elements (needed for perm->b^3/12 transmissivity);
		...
		if (line_string.find("$STORAGE_FUNCTION_EFFSTRESS") != std::string::npos)
		{
			...
			switch (storage_effstress_model)
			{
				case 1:
					...
					break;
				case 32: //FM2 //LU:10.2016 FM2 --- storage reading directly from .rfd curve
					in >> storage_effstress_model_value[0];  //LU 08.18 curve number, pressure vs storage
					//in >> storage_effstress_model_value[1];  //LU : curve number not used
					//in >> storage_effstress_model_value[2];  //LU :  curve number, pressure vs storage
					//in >> storage_effstress_model_value[3];  //LU : thickness of fault elements 
					break;
				default:
		...
		if (line_string.find("$STOR_FUNCTION_PRESSPLAS") != std::string::npos)
		{
			in.str(GetLineFromFile1(mmp_file));
			in >> storage_effstress_model;
			switch (storage_effstress_model)
			{
				case 1:
					in >> storage_effstress_model_value[0];
					break;
				case 32: //FM2 //LU:10.2018 FM2 --- storage reading directly from .rfd curve
					in >> storage_effstress_model_value[0];  //LU 08.18 curve number, pressure vs storage
					//in >> storage_effstress_model_value[1];  //LU : curve number not used
					//in >> storage_effstress_model_value[2];  //LU :  curve number, pressure vs storage
					//in >> storage_effstress_model_value[3];  //LU : thickness of fault elements 
					break;
				case 31: //FM1 //LU:10.2018 FM2 --- storage reading directly from .rfd curve
					in >> storage_effstress_model_value[0];  //LU 08.18 curve number, pressure vs storage
					//in >> storage_effstress_model_value[1];  //LU : curve number not used
					//in >> storage_effstress_model_value[2];  //LU :  curve number, pressure vs storage
					//in >> storage_effstress_model_value[3];  //LU : thickness of fault elements 
...
double CMediumProperties::Porosity(long number, double theta)
{
	...
	switch (porosity_model)
	{
		...
		case 12: // n = n0 + vol_strain, WX: 03.2011
			...
			break;
		case 13: // FM1 benchmark Task B decovalex		
		    porosity = PorosityFunctionEffStress(number, assem); // LU 12.2017
			break;
		case 15: // FM1 benchmark Task B decovalex, instead of dependency on stress, hydraulic aperture vs pressure	 			
		    porosity = PorosityFunctionEffStress(number, assem); //LU 12.2017
			break;
		case 14: // FM2 benchmark Task B decovalex				
		    porosity = PorosityFunctionEffStress(number, assem); // LU 12.2017
			break;
		case 16: // FM2 benchmark Task B decovalex pore pressure dependent permeability		
		    porosity = PorosityFunctionEffStress(number, assem); //LU 12.2017
			break;
		//case 13:
		//{
...
double CMediumProperties::Porosity(CElement* assem)
{
	...
	switch (porosity_model)
	{
		...
		case 12: // n = n0 + vol_strain
			...
			break;
		case 13: // FM1 benchmark Task B decovalex		
		    porosity = PorosityFunctionEffStress(number, assem_tmp); // LU 12.2017
			break;
		case 15: // FM1 benchmark Task B decovalex, instead of dependency on stress, hydraulic aperture vs pressure	 			
		    porosity = PorosityFunctionEffStress(number, assem_tmp); //LU 12.2017
			break;
...
double CMediumProperties::PermeabilityFunctionEffStress(long index, int nnodes, CFiniteElementStd* h_fem)
{
	...
	// functions from PERMEABILITY_FUNCTION_STRAIN to include strain calculations  LU 08.2017
	int idStrainP;
	double xelnodes[8]={0.} ; // LU 11.2017 to extract noodes coordinates from brick et similar elements
	double yelnodes[8]={0.} ; // LU 11.2017 to extract noodes coordinates from brick et similar elements
	double zelnodes[8]={0.} ; // LU 11.2017 to extract noodes coordinates from brick et similar elements
	double strainp_nodes[20] = {0.};
	double strainp, iindex = 0.;
	idStrainP = h_fem->dm_pcs->GetNodeValueIndex("STRAIN_PLS");
	for (int i = 0; i < nnodes; i++)
		strainp_nodes[i]= h_fem->dm_pcs->GetNodeValue(h_fem->dm_pcs->m_msh->ele_vector[index]->getNodeIndices()[i], idStrainP);
	strainp = h_fem->interpolate(strainp_nodes);
	...
	switch (permeability_effstress_model)
	{
		...
		case 1:
			...
			{
				for (i = 0; i < nnodes; i++)
					stress_nodes[i] = h_fem->dm_pcs->GetNodeValue(h_fem->dm_pcs->m_msh->ele_vector[index]->getNodeIndices()[i], stress_index[j]);
...
//------------------------------------------------------------------------
//12.(iv) PERMEABILITY_FUNCTION_PRESSURE_AND_PLS_STRAIN
//------------------------------------------------------------------------
// LU: permeability as function of pressure and plastic strain for liquid flow. 08.2012
double CMediumProperties::PermeabilityFunctionPressPlastic(long index, int nnodes, double pressure, CFiniteElementStd* h_fem)
{
	int idx, ii ,i, j, size;
	double perm_stress = 1.0;
	double faultthick = 1.0; //LU 08.2017
	...
	switch (permeability_effstress_model)
	{
		case 15: // FM1 benchmark Task B decovalex pore pressure dependent permeability		
		     Fem_Ele_Std->RealCoordinates(xyz); //determine coordinates of the element, check if inside radius of fracture
		     well_frac_square = permeability_effstress_model_value[6]*permeability_effstress_model_value[6];
		     well_dist_square=(xyz[0]-permeability_effstress_model_value[3])*(xyz[0]-permeability_effstress_model_value[3])+(xyz[1]-permeability_effstress_model_value[4])*(xyz[1]-permeability_effstress_model_value[4])+(xyz[2]-permeability_effstress_model_value[5])*(xyz[2]-permeability_effstress_model_value[5]);
		     if (well_dist_square < well_frac_square || strainp > 0.0)
		     {
		         strainp=1.0;
		         bstress = GetCurveValue((int)permeability_effstress_model_value[2], 0, pressure , &gueltig); // function for permeability vs pressure 
		     }
		     bshear = GetCurveValue((int)permeability_effstress_model_value[1], 0, strainp, &gueltig); // function for shear displ (pls strain times element thickness)
		     br = GetCurveValue((int)permeability_effstress_model_value[0], 0, 1, &gueltig); // constant - initial opening
		     faultthick = permeability_effstress_model_value[3]; // load element thickness as defined in .mmp
		     perm_stress  = (br + bshear + bstress)*(br + bshear + bstress)*(br + bshear + bstress)/ (12*faultthick);
		     break;	        
		case 16: // FM2 benchmark Task B decovalex pore pressure dependent permeability		
		     br = GetCurveValue((int)permeability_effstress_model_value[0], 0, 1, &gueltig); // constant - initial opening
		     bstress = GetCurveValue((int)permeability_effstress_model_value[2], 0, pressure , &gueltig); // function for permeability vs pressure 
		     ...
		     perm_stress  = (br + bshear + bstress)*(br + bshear + bstress)*(br + bshear + bstress)/ (12*faultthick);
		     break;	        
		case 18: // FM2 benchmark Task B decovalex pore pressure dependent permeability		
		     br = GetCurveValue((int)permeability_effstress_model_value[0], 0, 1, &gueltig); // constant - initial opening
		     if (strainp > 0.0)
		     {
		         bstress = GetCurveValue((int)permeability_effstress_model_value[2], 0, pressure , &gueltig); // function for permeability vs pressure 
		     ...
		     perm_stress  = (br + bshear + bstress)*(br + bshear + bstress)*(br + bshear + bstress)/ (12*faultthick);
		     break;	        
		case 22: // FM2 benchmark Task B decovalex pore pressure dependent permeability		
		     br = GetCurveValue((int)permeability_effstress_model_value[0], 0, 1, &gueltig); // constant - initial opening
		     bstress = GetCurveValue((int)permeability_effstress_model_value[2], 0, pressure , &gueltig); // function for permeability vs pressure 
		     bshear = GetCurveValue((int)permeability_effstress_model_value[1], 0, strainp, &gueltig); // function for shear displ (pls strain times element thickness)
		     //faultthick = permeability_effstress_model_value[3]; // load element thickness as defined in .mmp
		     perm_stress  = (br + bshear + bstress)*(br + bshear + bstress)/ (12);
		     break;	        
		case 21: // FM1 benchmark Task B decovalex pore pressure dependent permeability		
		     br = GetCurveValue((int)permeability_effstress_model_value[0], 0, 1, &gueltig); // constant - initial opening
		     if (strainp > 0) {bstress = GetCurveValue((int)permeability_effstress_model_value[2], 0, pressure , &gueltig);} // function for permeability vs pressure 
		     bshear = GetCurveValue((int)permeability_effstress_model_value[1], 0, strainp, &gueltig); // function for shear displ (pls strain times element thickness)
		     //faultthick = permeability_effstress_model_value[3]; // load element thickness as definbed in .mmp
		     perm_stress  = (br + bshear + bstress)*(br + bshear + bstress)/ (12);
		     break;
		case 32: // FM2 benchmark Task B decovalex pore pressure dependent permeability		
		     br = GetCurveValue((int)permeability_effstress_model_value[0], 0, 1, &gueltig); // constant - initial opening
		     bstress = GetCurveValue((int)permeability_effstress_model_value[2], 0, pressure , &gueltig); // function for permeability vs pressure 
		     bshear = GetCurveValue((int)permeability_effstress_model_value[1], 0, strainp, &gueltig); // function for shear displ (pls strain times element thickness)
		     faultthick = permeability_effstress_model_value[3]; // load element thickness as defined in .mmp
		     ...
		     //if (index = 383)
		     //		     { 
		     //		     std::cout << "Elem " << index << " Pressure " << value << " Permeability " << perm_stress << "\n";   //LU 07.10.2016 output
		     //		     } 
		     break;	        
		case 31: // FM1 benchmark Task B decovalex pore pressure dependent permeability		
		     br = GetCurveValue((int)permeability_effstress_model_value[0], 0, 1, &gueltig); // constant - initial opening
		     if (strainp > 0) {bstress = GetCurveValue((int)permeability_effstress_model_value[2], 0, pressure , &gueltig);} // function for permeability vs pressure 
		     bshear = GetCurveValue((int)permeability_effstress_model_value[1], 0, strainp, &gueltig); // function for shear displ (pls strain times element thickness)
		     faultthick = permeability_effstress_model_value[3]; // load element thickness as definbed in .mmp
...
//------------------------------------------------------------------------
//XX.(xx) POROSITY_FUNCTION_EFFSTRESS
//------------------------------------------------------------------------
// LU porosity updated with various relation decovalex
double CMediumProperties::PorosityFunctionEffStress(long index, CFiniteElementStd* assem)
{
	//double val = 0., 
    double vol_stress_temp = 0., stress_temp[3] = { 0. }, stress_nodes[20] = { 0. };
	// WW int idx_temp[3]={0}, ele_index, nnodes = assem->nnodes;
	int idx_temp[3] = { 0 }, idx, idStrainP, nnodes = assem->nnodes;
	// WW CRFProcessDeformation *dm_pcs = (CRFProcessDeformation *) this;
	double value_nodes[8] = { 0.0 };  // LU pressure value nodes and interpolated
	double value = 0.0;             // LU pressure value nodes and interpolated
	double pr = 0.0;                // LU to calculate b equivalent opening
	double pshear = 0.0;            // LU 08.2017
	double pstress = 0.0;           // LU 08.2017
	double well_dist = 0.0;         // LU 08.2017
	double xyz[3]; // LU 08.2017
	int dim = m_pcs->m_msh->GetCoordinateFlag() / 10;
	if (dim == 2)
		if (assem->axisymmetry)
			dim = 3;
	idx_temp[0] = assem->dm_pcs->GetNodeValueIndex("STRESS_XX");
	idx_temp[1] = assem->dm_pcs->GetNodeValueIndex("STRESS_YY");
	idx_temp[2] = assem->dm_pcs->GetNodeValueIndex("STRESS_ZZ");
	idx = m_pcs->GetNodeValueIndex("PRESSURE1"); // LU: test of reading pressures from nodes OGS
	double strainp_nodes[20] = { 0. };
	double strainp = 0.;
	idStrainP = assem->dm_pcs->GetNodeValueIndex("STRAIN_PLS");
	// WW ele_index = index;
	for (int j = 0; j < dim; j++)
	{
		for (int i = 0; i < nnodes; i++)
		{
			stress_nodes[i] = assem->dm_pcs->GetNodeValue(assem->dm_pcs->m_msh->ele_vector[index]->getNodeIndices()[i],
				idx_temp[j]);
			strainp_nodes[i] = assem->dm_pcs->GetNodeValue(assem->dm_pcs->m_msh->ele_vector[index]->getNodeIndices()[i], idStrainP);
		}
		strainp = assem->interpolate(strainp_nodes);
		stress_temp[j] = assem->interpolate(stress_nodes);
	}
	for (int j = 0; j < dim; j++)
		vol_stress_temp += stress_temp[j];
	//val += vol_strain_temp;	
	...
	case 13: // FM1 benchmark Task B decovalex		
			 ...
			 // stress[j] = h_fem->interpolate(stress_nodes);
			 // }	
		pr = GetCurveValue(porosity_model_values[0], 0, 1, &gueltig); // constant - initial opening
		pshear = GetCurveValue(porosity_model_values[1], 0, strainp, &gueltig); // function for pls_strain
		if (strainp > MKleinsteZahl)
		{
			pstress = GetCurveValue(porosity_model_values[2], 0, vol_stress_temp / 3., &gueltig); // function for eff. vol.stress
		}
		else
		{
			pstress = 0.0;
		}
		porosity = pr + pshear + pstress;
		//std::cout << "Element " << index << " . pr " << pr << " pshear " << pshear << " pstress " << pstress << " Stress eff. " << prin_str << " fac_perm_strain " << fac_perm_strain << "\n";	//LU 07.10.2016 output

		Fem_Ele_Std->RealCoordinates(xyz);
		//std::cout << "Element " << index << ". x " << xyz[0] << " y " << xyz[1] << " z " << xyz[2] << " fac_perm_strain " << fac_perm_strain << "\n";	//LU 07.10.2016 output
		if (porosity <= 0.)
		{
			porosity = pr + pshear + pstress;
		}
		well_dist = (xyz[0] - porosity_model_values[3])*(xyz[0] - porosity_model_values[3]) + (xyz[1] - porosity_model_values[4])*(xyz[1] - porosity_model_values[4]) + (xyz[2] - porosity_model_values[5])*(xyz[2] - porosity_model_values[5]);
		if (well_dist < porosity_model_values[6] * porosity_model_values[6])
		{
			//std::cout << "Old permeability. " << fac_perm_strain << " z " << xyz[2] << "\n";	//LU 07.10.2016 output
			pstress = GetCurveValue(porosity_model_values[2], 0, vol_stress_temp / 3., &gueltig); // function for eff. vol.stress
			pr = porosity_model_values[7];
			porosity = pr + pshear + pstress;
			//std::cout << "Well Element, open " << index << ". x " << xyz[0] << " y " << xyz[1] << " z " << xyz[2] << " 28 um + pstress " << pstress*1.e6 << "um \n";	//LU 07.10.2016 output
			//std::cout << "New permeability. " << fac_perm_strain << " x " << xyz[0] << " y " << xyz[1] << ",z " << xyz[2] << "\n";	//LU 07.10.2016 output
		}
		break;
	case 15: // FM1 benchmark Task B decovalex, instead of dependency on stress, porosity vs pressure	 	       
		pr = GetCurveValue(porosity_model_values[0], 0, 1, &gueltig); // constant - initial opening
		pshear = GetCurveValue(porosity_model_values[1], 0, strainp, &gueltig); // function for pls_strain
		if (strainp > MKleinsteZahl)
		{
			for (long ii = 0; ii < nnodes; ii++)
			{
				value_nodes[ii] = m_pcs->GetNodeValue(ii, idx);
				//nodes[ii] = MeshElement->nodes_index[ii];
				//double const* const pnt (nodes[ii]->getData()); // LU: read position of node ii of element
				//zelnodes[ii]=h_fem->dm_pcs->m_msh->ele_vector[index]->GetNode(ii)->getData()[2]; //pnt[0];
				//yelnodes[ii]=h_fem->dm_pcs->m_msh->ele_vector[index]->GetNode(ii)->getData()[1]; //pnt[0];
				//xelnodes[ii]=h_fem->dm_pcs->m_msh->ele_vector[index]->GetNode(ii)->getData()[0]; //pnt[0];
				//std::cout << "Node " << ii << " Pressure " << value_nodes[ii] << " position: x " << xelnodes[ii] << " y " << yelnodes[ii] << " z " << zelnodes[ii] << "\n";	//LU 07.10.2016 output
			}
			value = assem->interpolate(value_nodes);
			pstress = GetCurveValue(porosity_model_values[2], 0, value, &gueltig);
		}
		else
		{
			pstress = 0.0;
	...
	case 14: // FM2 benchmark Task B decovalex		
		pr = GetCurveValue(porosity_model_values[0], 0, 1, &gueltig); // constant - initial opening
		pstress = GetCurveValue(porosity_model_values[2], 0, vol_stress_temp / 3., &gueltig); // function for eff. vol.stress
		pshear = GetCurveValue(porosity_model_values[1], 0, strainp, &gueltig); // function for shear displ (pls strain times element thickness)
		porosity = pr + pshear + pstress;
		break;
	case 16: // FM2 benchmark Task B decovalex pore pressure dependent porosity		
		idx = m_pcs->GetNodeValueIndex("PRESSURE1"); // LU: test of reading pressures from nodes OGS
													 // material_group = ElGetElementGroupNumber(index); // LU: read material group of element
		for (long ii = 0; ii < nnodes; ii++)
		{
			value_nodes[ii] = m_pcs->GetNodeValue(ii, idx);
			//nodes[ii] = MeshElement->nodes_index[ii];
		...
		value = assem->interpolate(value_nodes);
		pr = GetCurveValue(porosity_model_values[0], 0, 1, &gueltig); // constant - initial opening
		pstress = GetCurveValue(porosity_model_values[2], 0, value, &gueltig); // function for permeability vs pressure 
		pshear = GetCurveValue(porosity_model_values[1], 0, strainp, &gueltig); // function for shear displ (pls strain times element thickness)
		porosity = pr + pshear + pstress;
		break;
	}
...
double CMediumProperties::StorageFunction(long index, double* gp, double theta)
{
	...
	gp = gp;
	index = index;
	// double value_nodes[20] = { 0.0 };  // LU pressure value nodes and interpolated
	// double value = 0.0;             // LU pressure value nodes and interpolated
	// int gueltig;
	// int nn, i, Type;
	// int idummy;
...
/* Funktion der effektiven Spannung und des Drucks in Elementmitte
		...
		case 32:
			// LU see StorageFunctionPressure
...
double CMediumProperties::StorageFunctionEffStress(long index, int nnodes, CFiniteElementStd* h_fem)
{
	...
	switch (storage_effstress_model)
	{
		...
		case 1:
			...
			break;
		case 32: // BMT benchmark Task B decovalex pore pressure dependent storage (opening intrinsic)		
			//idx = m_pcs->GetNodeValueIndex("PRESSURE1"); // LU: test of reading pressures from nodes OGS
			// material_group = ElGetElementGroupNumber(index); // LU: read material group of element
			//for (long ii = 0; ii < nnodes; ii++)
		     	//   	     { 
		     	//   	     stress_nodes[ii] = m_pcs->GetNodeValue(ii, m_pcs->GetNodeValueIndex("PRESSURE1")); //stress in the name, but it is pressure
		     		     //zelnodes[ii]=h_fem->dm_pcs->m_msh->ele_vector[index]->GetNode(ii)->getData()[2]; //pnt[0];
		     		     //yelnodes[ii]=h_fem->dm_pcs->m_msh->ele_vector[index]->GetNode(ii)->getData()[1]; //pnt[0];
		     		     //xelnodes[ii]=h_fem->dm_pcs->m_msh->ele_vector[index]->GetNode(ii)->getData()[0]; //pnt[0];
		     	//   	     } 
			//value = h_fem->interpolate(stress_nodes);
			//value = h_fem->node_max(stress_nodes); //stress in the name, but it is pressure
			// permeability from curve with minimum (absolute value) principal effective stress as input
			storage_stress = GetCurveValue((int)storage_effstress_model_value[0], 0, 500, &i);
			break;
		default:
			break;
	}
...
// LU:08.2018 storage function pressure
double CMediumProperties::StorageFunctionPressPlastic(long index, int nnodes, double pressure, CFiniteElementStd* h_fem)
{
	int idx, gueltig;
	double storage = 1.0;
	//double value = 1.0;
	//double value_nodes[20] = {0.};
	storage = GetCurveValue((int)storage_effstress_model_value[0], 0, pressure, &gueltig);
	//std::cout << " StorageFunctionPressPlastic curve: " << storage_effstress_model_value[0] << "with pressure " << pressure  << "val now "<<storage<<"\n"; 
