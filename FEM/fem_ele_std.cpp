...
double CFiniteElementStd::CalCoefMass()
{
	...
	switch (PcsType)
	{
		...
		case EPT_LIQUID_FLOW: // Liquid flow
			...
			// if (MediaProp->storage_model == 32) // Add MediaProp->storage_model.  29.09.2011. WW
			// {
				// CFiniteElementStd* h_fem;
				// h_fem = this;
				// val = MediaProp->StorageFunctionPressure(Index, nnodes, h_fem);			
			// }
			...
			{
				poro_val = MediaProp->Porosity(Index, pcs->m_num->ls_theta);
                double porosity = MediaProp->porosity_model_values[0];
				val += poro_val * drho_dp_rho;
				//std::cout << "Storage used is "<< val  <<" \n";
			}

			// AS,WX: 08.2012 storage function eff stress
			if (MediaProp->storage_effstress_model > 0)
			{
				//std::cout << " storage function eff stress recognized  \n";
				double storage_effstress = 1.;
				CFiniteElementStd* h_fem;
				h_fem = this;
				if (MediaProp->storage_effstress_model > 12)
				{
					ipress = interpolate(NodalVal1);
					storage_effstress = MediaProp->StorageFunctionPressPlastic(Index, nnodes, ipress , h_fem);
					val = storage_effstress;
				//std::cout << " storage function eff stress recognized, model 32 (" << MediaProp->storage_effstress_model << "larger than 12), with pressure " << ipress  << "val now "<< val <<"\n";;
				}
				else
				{
					storage_effstress = MediaProp->StorageFunctionEffStress(Index, nnodes, h_fem);
					val += storage_effstress;
				//std::cout << " storage function eff stress recognized, old model" << "\n";
				}
			}
	...
	//std::cout << "final storage given  " << val  << " with storage_effstress_model  " << MediaProp->storage_effstress_model << "\n";
	return val;
}
...
void CFiniteElementStd::CalCoefLaplace(bool Gravity, int ip)
{
	...
	double mat_fac = 1.0;
	double ref_tensor = 1.0; //LU to evaluate anistropic & orthohtropic perm
	double Dpv = 0.0;
	double poro = 0.0;
	double tort = 0.0;
	double humi = 1.0;
	double rhow = 0.0;
        double perm_effstress_k = 0.0; //for evaluation of laplacian
	double* tensor = NULL;
	...
	switch (PcsType)
	{
		...
		case EPT_LIQUID_FLOW: // Liquid flow
			...
			tensor = MediaProp->PermeabilityTensor(Index);
			ref_tensor = tensor[0]; // LU 09.2018 to scale permeability
			// AS:08.2012 permeability function eff stress
			if (MediaProp->permeability_effstress_model > 0)
			{
				CFiniteElementStd* h_fem;
				h_fem = this;
				if (MediaProp->permeability_effstress_model > 12) //LU: 08.2018, correct pressure reading here with OpenMP
				{
						ipress = interpolate(NodalVal1);
						//std::cout << "ELEMENT " << Index << " Pressure " << ipress << "\n";	//LU 03.2017 output
						fac_perm = MediaProp->PermeabilityFunctionPressPlastic(Index, nnodes, ipress, h_fem);
						if (ipress <0)
						{
                        ipress=-1;
						//std::cout << "Warning ! Pressure of element " << Index << " is negative: " << ipress << " Pa ).\n";	//LU 03.2017 output
						}
						//std::cout << "ELEMENT " << Index << " Permeability " << fac_perm << "\n";	//LU 03.2017 output
				}
				else
				{
				fac_perm = MediaProp->PermeabilityFunctionEffStress(Index, nnodes, h_fem);
				}//std::cout << "ELEMENT " << Index << " Permeability " << fac_perm << "\n";	//LU 03.2017 output
			}
			...
			{ // 13= FM1 14=FM2 15&16 the same but with permeability depending on pressure instead of effective stress, 32 is preserving permeability instead of transmissivity for a single fracture flow
				for (size_t i = 0; i < dim * dim; i++)
				{
				   if (tensor[i] == 0) 
				       {
				          perm_effstress_k = 1.0 ; // AS:perm. dependent eff stress.
				       }
				   else
				       {
				          // std::cout << " permeability tensor type " <<MediaProp->permeability_tensor_type << " if > 0 orthotropic (=1) or anisotropic (=2) and it must be normalized in input .mmp file \n";	//LU 07.10.2016 output
				...
				mat[i] = tensor[i] / mat_fac * perm_effstress_k * k_rel ; // AS:perm. dependent eff stress.
				//if (variables[0] > 500001)
                //{
					//for (int i = 0; i < nnodes; i++)
					//	pressureHead[i] = pcs->GetNodeValue(nodes[i], "PRESSURE1"); //LU false name, just as a try
                    //std::cout << " Laplacian coeff mat(i) " <<mat[i] << " pressure" << variables[0] << " factor (br+bsh+bstr)^3/12 thick " << perm_effstress_k <<" \n";	//LU 07.10.2016 output
				//}	
                }
			}
			else
			{
				for (size_t i = 0; i < dim * dim; i++)
				{
				   mat[i] = tensor[i] / mat_fac * fac_perm * k_rel; // LU generic fac_perm.
				   //if (i == 0)
				   //   std::cout << " Laplacian coeff mat(i) " <<mat[i] << " tensor(i) " << tensor[i] <<" \n";	//LU 07.10.2016 output
				}
			}
